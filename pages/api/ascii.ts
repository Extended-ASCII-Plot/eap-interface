...
export default async function Ascii(_req: NextApiRequest, res: NextApiResponse) {
  const { buffer } = await getAscii()
  res.send(Buffer.from(buffer).toString('base64'))
}

export async function getAscii(): Promise<BigUint64Array> {
